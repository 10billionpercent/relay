...
export default function Sidebar(props: SidebarProps) {
  ...
  // Always close sidebar FIRST, then do the action
  const selectConversationAndClose = (id: string) => {
    closeMobileSidebar(); // ← close BEFORE any state changes
    setRenamingConvId(null);
    onSelectConversation(id);
  };

  return (
    // ❌ Overlay removed — ChatPage already provides it
    <motion.aside
      layout
      animate={{ width: sidebarWidth }}
      transition={{ type: "spring", stiffness: 300, damping: 30 }}
      className={`fixed lg:static inset-y-0 left-0 z-50 flex flex-col bg-[#1a1d21] border-r border-[#2a2d33] overflow-hidden ${
